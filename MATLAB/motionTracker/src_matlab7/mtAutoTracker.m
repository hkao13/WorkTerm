                ...
                if ~strcmp( trak.method, 'manual' ),
                    ...
                    if tolMarkerSize == 0 | ...
                        ...
                        set(guiH.commentTxt, 'String', ...
                            sprintf('%s Marker:\nRIGHT CLICK => ACCEPT,\nLEFT CLICK => NEW CENTER,\nClick outside video frame to cancel',...
                            gv.mrkrLabels{iMarker}));
                                                
                        [xCentMouse, yCentMouse, butn] = ginput(1);
                        if xCentMouse < 0 | xCentMouse > gv.nHp | yCentMouse < 0 | yCentMouse > gv.nVp,
                            %user clicked outside video frame -> tracking
                            %canceled
                            mTraj(iFrame, iMarker*2-1:iMarker*2) = NaN;
                            %gv.boxH(iMarker) = NaN;
                            disp('Tracking canceled');
                            mtGUI('stop');
                            return;                            
                        elseif butn == 1,
                            mTraj(iFrame, iMarker*2-1:iMarker*2) = [xCentMouse, yCentMouse];
                        elseif butn == 3,
                            disp('Position okay, continue tracking')
